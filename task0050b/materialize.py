...
def main() -> int:
    ...
    if plan["source_commit"] != PINNED_COMMIT:
        raise RuntimeError("TASK0050B_PINNED_COMMIT_GATE_FAILED")
    ...
    if len(dev_plan) != 1688 or len(dev_ids) != 1688 or len(holdout_ids) != 101 or dev_ids & holdout_ids:
        raise RuntimeError("TASK0050B_MEMBERSHIP_GATE_FAILED")
    ...
    if set(metadata_by_id) != dev_ids:
        raise RuntimeError("TASK0050B_METADATA_GATE_FAILED")

    observations=[]; labels=[]
    total_bytes=total_events=exact=positives=0
    for item in dev_plan:
        ...
        if git_blob_sha1(body) != expected:
            raise RuntimeError(f"TASK0050B_RAW_BLOB_GATE_FAILED:{mid}")
        events=json.loads(body)
        if not isinstance(events,list) or not events:
            raise RuntimeError(f"TASK0050B_RAW_CONTENT_GATE_FAILED:{mid}")
        lab=label_event_file(body,item["event_source_path"],expected)
        if lab["status"] == "censored":
            raise RuntimeError(f"TASK0050B_CENSORED_MATCH:{mid}")
        target=bool(lab["target_label"])
        observations.append(observation(metadata_by_id[mid],target))
        labels.append({
            "sample_id":f"statsbomb:{mid}","provider_match_id":mid,
            "target":"football.match.goal_in_first_10_minutes.v1","target_label":target,
    ...
    state=ResearchState(); pending10=[]; pendingfull=[]; rows=[]
    transition_violations=[]; provenance_violations=[]
    ...
    for target in ordered:
        ...
        if len(features) != 10 or len(metadata) != 4:
            raise RuntimeError(f"TASK0050B_FEATURE_COUNT_GATE_FAILED:{target.sample_id}:{len(features)}:{len(metadata)}")
        provenance_violations.extend({"sample_id":target.sample_id,**v} for v in audit_row(target,features+metadata,by_sample))
        rows.append({
            "sample_id":target.sample_id,"competition_id":target.competition_id,"season_id":target.season_id,
            "prediction_as_of":target.kickoff.isoformat(),
            "predictive_features":[feature_dict(f) for f in features],
    ...
    if transition_violations:
        raise RuntimeError(f"TASK0050B_TRANSITION_LEAKAGE_GATE_FAILED:{transition_violations[:5]}")
    if provenance_violations:
        raise RuntimeError(f"TASK0050B_PROVENANCE_LEAKAGE_GATE_FAILED:{provenance_violations[:5]}")
    expected_sids={f"statsbomb:{x}" for x in dev_ids}
    if len(rows)!=1688 or {r["sample_id"] for r in rows} != expected_sids:
        raise RuntimeError("TASK0050B_ROW_MEMBERSHIP_GATE_FAILED")

    availability=defaultdict(lambda:defaultdict(int))
    schedule_null_count=0; removed_unstable_present=[]; process_block_present=[]
    numeric_schedule_keys={
        "football.prematch.rest_days_capped_diff.v4",
        "football.prematch.recent_fixture_count_7d_diff.v4",
    }
    for row in rows:
        pkeys=[f["key"] for f in row["predictive_features"]]
        mkeys=[f["key"] for f in row["metadata_features"]]
        if pkeys != list(PREDICTIVE_FEATURE_KEYS):
            raise RuntimeError(f"TASK0050B_PREDICTIVE_ORDER_GATE_FAILED:{row['sample_id']}")
        if mkeys != list(METADATA_KEYS):
            raise RuntimeError(f"TASK0050B_METADATA_ORDER_GATE_FAILED:{row['sample_id']}")
        for f in row["predictive_features"]:
            availability[f["key"]][f["availability"]] += 1
            if f["key"] in numeric_schedule_keys and f["value"] is None:
                schedule_null_count += 1
            if f["key"] in REMOVED_UNSTABLE_KEYS:
                removed_unstable_present.append(f["key"])
            if "xg" in f["key"].lower() or "shots" in f["key"].lower():
                process_block_present.append(f["key"])
    if schedule_null_count != 0:
        raise RuntimeError(f"TASK0050B_SCHEDULE_NULL_GATE_FAILED:{schedule_null_count}")
    if removed_unstable_present:
        raise RuntimeError(f"TASK0050B_REMOVED_UNSTABLE_PRESENT:{removed_unstable_present[:3]}")
    if process_block_present:
        raise RuntimeError(f"TASK0050B_PROCESS_BLOCK_PRESENT:{process_block_present[:3]}")

    write_json("development-explicit-maturity-v4-feature-dataset-v1.json",{
        "schema_version":1,"dataset_id":"football.prematch.explicit_maturity_v4.development.v1",
        "source_commit":PINNED_COMMIT,"scope":"development_only","row_count":1688,
        "predictive_feature_count":10,"metadata_feature_count":4,
        "predictive_feature_keys":list(PREDICTIVE_FEATURE_KEYS),"metadata_feature_keys":list(METADATA_KEYS),
        "availability_contract":{
            "prediction_anchor":"target kickoff",
            "first10_history":"source kickoff < target kickoff AND source kickoff + 10 minutes <= target kickoff",
            "fullmatch_history":"source kickoff < target kickoff AND source kickoff + 3 hours <= target kickoff",
            "schedule_numeric_null_policy":"always defined; neutral fallback 0.0 before both teams have 3 completed prior matches",
            "schedule_maturity_threshold":3,
        },
        "removed_unstable_features":list(REMOVED_UNSTABLE_KEYS),
        "removed_current_blocks":["opponent_relative_process/xG/shots"],
        "schedule_numeric_null_count":0,
    ...
    write_json("development-first10-labels-task0050b.json",{
        "schema_version":1,"scope":"development_only","target":"football.match.goal_in_first_10_minutes.v1",
    ...
    write_json("raw-verification-receipt.json",{
        "status":"VERIFIED","source_commit":PINNED_COMMIT,"match_count":1688,
        "exact_git_blob_verified_count":exact,"raw_source_bytes":total_bytes,"raw_event_count":total_events,
    })
    receipt={
        "task_id":"TASK-0050B","status":"VERIFIED","source_commit":PINNED_COMMIT,
        "development_match_count":1688,"predictive_feature_row_count":1688,"predictive_feature_count":10,"metadata_feature_count":4,
        "exact_git_blob_verified_count":exact,"raw_source_bytes":total_bytes,"raw_event_count":total_events,
        ...
        "schedule_numeric_null_count":0,"explicit_schedule_maturity_control_count":2,
        "removed_sign_unstable_features_absent":True,"removed_opponent_relative_process_block":True,
        "holdout_match_count":101,"holdout_rows_materialized":0,"holdout_labels_opened":False,"holdout_evaluated":False,
        "model_training_performed":False,
    }
    write_json("TASK-0050B-VERIFIED-receipt.json",receipt)
